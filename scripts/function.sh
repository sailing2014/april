...
function do_crontab() {
    ...
    if [ "$2" = 'install' -o "$2" = 'update' -o "$2" = 'restore' ]; then
        sudo crontab -u $_user $_crontab || return 1
...
# Check Port
#-----------------------------------------------------------
function httpd_check_port()
{
    port=$((USERID*10))
    ss -ltnp | grep -v httpd | awk '{print $4}' | awk -F':' '{print $NF}'  | grep -w ${port}  && return 1  || return 0
}
...
function do_httpd() {
    ...
    if [ -f /home/$1/conf/httpd.conf ];then
        ...
        [ ! -f /home/$1/conf/nginx.conf ] || return 1
        [ ! -f /home/$1/conf/nginx.$1.conf ] || return 1
    fi
    ...
    case "$2" in
        install)
            if [ -f $_httpd_local ]; then
                ...
                httpd_check_port || return 1
                pgrep -f "/usr/sbin/httpd -k graceful"  && sleep 5
                pgrep -f "/usr/sbin/httpd -k graceful"  && errmsg "/usr/sbin/httpd -k graceful still exist" && return 1
...
function install_nginx_php()
{
    ...
    unlink /etc/httpd/conf.d/httpd.$1.conf >/dev/null 2>&1
    if [ $(which httpd) ];then
        sudo service httpd configtest || return 1   
        pgrep -f httpd && sudo service httpd graceful
    fi
    [ ! -f /etc/php-fpm.d/www.conf.bak ] ||  sudo mv /etc/php-fpm.d/www.conf.bak /etc/php-fpm.d/www.conf
    [ -f /etc/php-fpm.d/www.conf ] ||  return 1
    sudo sed -i "s/pm.max_children.*/pm.max_children = 30/" /etc/php-fpm.d/www.conf
    sudo sed -i "s/pm.start_servers.*/pm.start_servers = 2/" /etc/php-fpm.d/www.conf
    sudo sed -i "s/pm.min_spare_servers.*/pm.min_spare_servers = 2/" /etc/php-fpm.d/www.conf
    sudo sed -i "s/pm.max_spare_servers.*/pm.max_spare_servers = 5/" /etc/php-fpm.d/www.conf
    ...
    [  -f /etc/rsyslog.d/php-fpm_rsyslog.conf ] || sudo cp /home/$1/conf/php-fpm_rsyslog.conf /etc/rsyslog.d/php-fpm_rsyslog.conf
...
function install_nginx_rtmp(){
    rpm -qa | grep  epel-release || sudo yum install -y epel-release >/dev/null
    rpm -qa | grep nux-dextop-release || sudo rpm -Uvh http://li.nux.ro/download/nux/dextop/el6/x86_64/nux-dextop-release-0-2.el6.nux.noarch.rpm || return 1
    which ffmpeg || sudo yum install -y ffmpeg ffmpeg-devel >/dev/null
    which ffmpeg || return 1
    which php-fpm || sudo yum install -y php-fpm >/dev/null
    which php-fpm || return 1
    which php || yum -y install php php-cli php-mcrypt php-mbstring php-mysql php-pdo php-xml php-pear php-pecl-memcache php-pecl-apc php-bcmath php-gd >/dev/null
    which php || return 1
    curl -o /etc/yum.repos.d/qiwo.repo yum.qiwocloud1.com/qiwo.repo
    php -m | grep phalcon || yum install -y phalcon >/dev/null
    php -m | grep phalcon || return 1 
    rpm -qa | grep -E nginx-[0-9] && return 1
    sudo yum install -y nginx-rtmp >/dev/null
    nginx -v || return 1
    chkconfig --add php-fpm  && chkconfig php-fpm on
    chkconfig --add nginx && chkconfig nginx on
    grep "51200" /etc/nginx/nginx.conf || sed -i 's#worker_connections  1024;#worker_connections  51200;#' /etc/nginx/nginx.conf 
    grep -i ^expose_php.*Off /etc/php.ini || sed -i "s/^expose_php.*/expose_php = Off/" /etc/php.ini
    pgrep -f httpd && return 1
    [ ! -f /etc/php-fpm.d/www.conf.bak ] ||  sudo mv /etc/php-fpm.d/www.conf.bak /etc/php-fpm.d/www.conf
    [ -f /etc/php-fpm.d/www.conf ] ||  return 1
    sudo usermod -G1001 apache
    sudo usermod -G1001 nginx
...
#   $3: environemt: { developing | staging | product ....}
#---------------------------------------------------------------------------------------------
function do_nginx_rtmp(){
    local _nginxrtmp_local="/home/$1/conf/nginxrtmp.$1.conf"
    local _nginxrtmp_conf="/etc/nginx/conf.d/nginxrtmp.$1.conf"
    local _rtmp_local="/home/$1/conf/rtmp.$1.conf"
    local _rtmp_conf="/etc/nginx/rtmp.d/rtmp.$1.conf"
    [ ! -f /home/$1/conf/nginxrtmp.conf ] && [ ! -f $_nginxrtmp_local ]  && return 0
    [ -f /home/$1/conf/nginxrtmp.conf ]  &&  sudo -u $1 mv /home/$1/conf/nginxrtmp.conf $_nginxrtmp_local
    [ ! -f /home/$1/conf/rtmp.conf ] && [ ! -f $_rtmp_local ] && return 0
    [ -f /home/$1/conf/rtmp.conf ] && sudo -u $1 mv /home/$1/conf/rtmp.conf $_rtmp_local
    install_nginx_rtmp $1  || return 1
    case "$2" in
        install | update )
            if [ -f $_nginx_local ]; then
                sudo ln -fsn $_nginxrtmp_local $_nginxrtmp_conf
                sudo ln -fsn $_rtmp_local $_rtmp_conf
                sudo nginx -t || return 1
                pgrep -f 'nginx: master process' || sudo service nginx restart
                pgrep -f "php-fpm: master process" || sudo service php-fpm restart
                sudo service php-fpm reload || return 1
        ...
        restore )
            ...
            ;;
        uninstall)
            if [ -f $_nginxrtmp_conf ]; then
                unlink $_nginxrtmp_conf
                unlink $_rtmp_conf
                sudo service php-fpm reload || return 1
                sudo service nginx reload || return 1
            fi
...
# install php5.4 & nginx
#-------------------------------------------------------------
#
function install_php5(){
    rpm -qa | grep epel-release  || sudo yum install -y epel-release >/dev/null
    rpm -qa | grep centos-release-SCL || sudo yum install -y centos-release-SCL >/dev/null
    rpm -qa | grep remi-php54more-epel || sudo rpm -Uvh https://www.softwarecollections.org/repos/remi/php54more/epel-6-x86_64/noarch/remi-php54more-epel-6-x86_64-1-2.noarch.rpm
    curl -o /etc/yum.repos.d/qiwo.repo yum.qiwocloud1.com/qiwo.repo || return 1
    source /etc/profile
    php -v | grep 5.3 && return 1
    php -m | grep mongo &&  php -m | grep phalcon  && php -m | grep infosec || sudo yum install -y php54 php54-php-mongo php54-phalcon infosec php54-php-fpm php54-php-mbstring php54-php-gd php54-php-mcrypt >/dev/null || return 1
    which nginx || sudo yum install -y nginx >/dev/null || return 1
    [ -f /etc/nginx/conf.d/default.conf ] && sudo mv /etc/nginx/conf.d/default.conf /etc/nginx/conf.d/default.conf.bak
    sudo ln -fsn /opt/rh/php54/enable  /etc/profile.d/enable.sh
    source /etc/profile
    sudo chkconfig php54-php-fpm on
    sudo sed -i "s/pm.max_children.*/pm.max_children = 30/"  /opt/rh/php54/root/etc/php-fpm.d/www.conf
    sudo sed -i "s/pm.start_servers.*/pm.start_servers = 2/"  /opt/rh/php54/root/etc/php-fpm.d/www.conf
    sudo sed -i "s/pm.min_spare_servers.*/pm.min_spare_servers = 2/"  /opt/rh/php54/root/etc/php-fpm.d/www.conf
    sudo sed -i "s/pm.max_spare_servers.*/pm.max_spare_servers = 5/"  /opt/rh/php54/root/etc/php-fpm.d/www.conf
    sudo service php54-php-fpm restart
    grep "51200" /etc/nginx/nginx.conf || sed -i 's#worker_connections  10240;#worker_connections  51200;#' /etc/nginx/nginx.conf
    grep -i ^expose_php.*Off /etc/php.ini || sed -i "s/^expose_php.*/expose_php = Off/" /opt/rh/php54/root/etc/php.ini
    pgrep -f httpd && return 1
    sudo usermod -G1001 apache
    sudo usermod -G1001 nginx
...
#   $3: environemt: { developing | staging | product ....}
#---------------------------------------------------------------------------------------------
function do_nginx_php54(){
    local _nginxphp54_local="/home/$1/conf/nginxphp54.$1.conf"
    local _nginxphp54_conf="/etc/nginx/conf.d/nginxphp54.$1.conf"
    [ ! -f /home/$1/conf/nginxphp54.conf ] && [ ! -f $_nginxphp54_local ]  && return 0
    [ -f /home/$1/conf/nginxphp54.conf ] && sudo -u $1 mv /home/$1/conf/nginxphp54.conf $_nginxphp54_local
    install_php5 $1 || return 1
    case "$2" in
        install | update )
            if [ -f $_nginxphp54_local ]; then
                sudo ln -fsn $_nginxphp54_local $_nginxphp54_conf
                sudo nginx -t || return 1
                pgrep -f 'nginx: master process' || sudo service nginx restart
                pgrep -f "php-fpm: master process" || sudo service php-fpm restart
                sudo service php54-php-fpm reload || return 1
                sudo service nginx reload || return 1
            fi
            ;;
        restore )
            if [ -f $_nginxphp54_conf ]; then
               sudo service php54-php-fpm reload || return 1
               sudo service nginx reload || return 1
            fi
            ;;  
        uninstall )
            sudo unlink $_nginxphp54_conf
            sudo service nginx reload || return 1
            ;;
        * )
            return 1
...
# install pm2
# if use  pm2 --version,That command that executed by salt-stack would be have an high cpu 
# we only support node v6.10.3 
#-------------------------------------------------------------
function install_pm2() {
    ver=""
    which node && ver=$(node -v)
    [ X${ver} != X ] && [ "X${ver}" != "Xv6.10.3" ] && return 1
    which node || rpm -Uvh https://rpm.nodesource.com/pub_6.x/el/6/x86_64/nodesource-release-el6-1.noarch.rpm
    sed -i '/priority/d'  /etc/yum.repos.d/nodesource-el.repo
    sed -i '/]/a\priority = 40' /etc/yum.repos.d/nodesource-el.repo 
    which node >/dev/null 2>&1 || sudo yum install -y nodejs-6.10.3 --disablerepo=epel || return 1
    which pm2 >/dev/null 2>&1 || sudo npm install pm2@latest -g || return 1
    [ "X$(node -v)" == "Xv6.10.3" ]  ||  return 1
}
...
function build_pm2app_service() {
    ...
    which pm2 || return 1
    unlink /etc/init.d/${_pm2srv}
    pm2 startup centos6 systemv -u ${_user} --hp /home/${_user}
    /bin/cp /etc/init.d/${_pm2srv} ${_pm2init}
}
...
function do_pm2app_service() {
    ...
    [ -f $_pm2init ] && chmod 777 $(readlink -e $_pm2init)
...
function install_app() {
    ...
    do_nginx_rtmp   $_user $_action || return 8
    do_nginx_php54  $_user $_action || return 9
    sudo -u $_user find $_src -type d -exec chmod 750 {} \;
    sudo -u $_user find $_src -type f -exec chmod 640 {} \;
    [ -f /home/$_user/conf/pm2-$_user ] && chmod 777 /home/$_user/conf/pm2-$_user
...
function uninstall_app() {
     if [
        ...
        do_nginx_rtmp   $1 uninstall $2 || return 8
        do_nginx_php54  $1 uninstall $2 || return 9
    fi
}
